  ...
  <div class="toolbar" :class="{ 'only-close': onlyClose }">
    ...
    recordRef?.scrollToNode(current.id)
        ...
        <el-space alignment="flex-start">
          <span class="t-gray f-12  m-b-4">
            总计
            <span class="t-red f-12 m-b-4">{{ totalCount }}</span>
            条
          </span>
          <span class="t-gray f-12 m-b-4">
            今日
            <span class="t-red f-12 m-b-4">{{ todayCount }}</span>
            条
          </span>
        ...
        <el-tree-v2 :data="treeData" :props="treeProps" :default-expanded-keys="treeExpand" ref="recordRef" indent="4">
          <template #default="{ node }">
            ...
            <div v-if="node.isLeaf" class="flex gap-4 flex-between w-full" style="padding-right: 10px;">
              <el-space>
                <span class="t-gray t-no-select">{{ dateTimeFmt(node.data.create_time, 6) }}</span>
                <div v-if="node.label.split('<+++>').length > 1">
                  <span class="t-primary t-no-select"> {{ node.label.split('<+++>')[0] }}</span>
                  <span class="t-black t-no-select"> {{ node.label.split('<+++>')[1] }}</span>
                </div>
                <span v-else class="t-primary t-no-select">{{ node.label }}</span>

              </el-space>
              <el-space>
                <el-button type="danger" link v-if="!configStore.mode" @click="() => {
    showDebug = true
    ...
    showDebugTitle = node.data.url
        ...
        <template #reference>
          <el-button link @click="showOperateRecords">
            <el-space alignment="center" v-if="showRecords && current">
              <span class="t-black f-12  t-no-select">{{ dateTimeFmt(current.create_time / 1000, 6) }}</span>
              <div v-if="current.title.split('<+++>').length > 1">
                <span class="t-primary t-no-select f-12"> {{ current.title.split('<+++>')[0] }}</span>
                <span class="t-black t-no-select f-12"> {{ current.title.split('<+++>')[1] }}</span>
              </div>
              <span v-else class="t-primary t-no-select">{{ current.title }}</span>
            ...
            <span v-else-if="totalCount === 0" class="t-gray f-12 t-no-select">暂无记录</span>
      ...
      <el-space class="drag">
        ...
        <span class="t-gray f-12 b-500 t-no-select">
